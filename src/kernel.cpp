...
//
// $Id: kernel.cpp 13 2008-04-18 08:02:14Z eg1981 $
...
void Kernel::handlePropertyNotify(XPropertyEvent *event) {
    ...
    if (event->atom == Atoms::NCWM_STATUSTEXT ||
        event->atom == Atoms::NCWM_METERTEXT)
    {
        focusedMonitor()->updateBars();
        return;
    }
    else if (event->atom == Atoms::NCWM_ACTIONCMD) {

        // TODO
        // action[+arg1[...]]
        string actionCmd;
        if (XCORE->textProperty(defaultRootWindow(),
                    Atoms::NCWM_ACTIONCMD, &actionCmd))
        {
            string bind, arg;
    ...
    else if (event->atom == Atoms::NCWM_PRETTYPRINT_REQUEST) {
        ...
        XCORE->setTextProperty(display_, defaultRootWindow(),
                &property, Atoms::NCWM_PRETTYPRINT_RESPONSE);
    }
}
